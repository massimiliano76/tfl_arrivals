...
def test_construction():
    ...
    arr = arrival_data(234, 42, "Acton Town", expected, ttl)
    assert arr.vehicle_id == 234
    assert arr.stop_id == 42
...
def test_to_string():
    ...
    arr = arrival_data(234, 42, "Uxbridge", arrive_at, ttl, now=dummy_now)
    ...
    arr = arrival_data(234, 42, "Elephand and Castle", arrive_at, ttl, now=dummy_now)
    assert str(arr) == "Elephand and Castle            15"

    arrive_at = datetime(2017, 4, 12, 17, 24, 25)
    ttl = datetime(2017, 4, 12, 17, 10, 40, 0)
    dummy_now = lambda : datetime(2017, 4, 12, 17, 10, 11)
    arr = arrival_data(234, 42, "Very-very long stop names should be cropped", arrive_at, ttl, now=dummy_now)
...
def test_expected_in_seconds_mocked():
    ...
    arr = arrival_data(234, 0, "X", expected, dummy_now(), dummy_now)
...
def test_expected_in_seconds_real():    
    ...
    expected = datetime.now() + delta
    arr = arrival_data(243, 0, "X", expected, expected)
...
def test_is_expired_mocked():
    ...
    expected = dummy_now() + delta
    ttl = dummy_now() + delta
    arr = arrival_data(234, 0, "X", expected, ttl, dummy_now)
